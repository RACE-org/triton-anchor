...
// fantgpu: spirv version SPV_ENV_UNIVERSAL_1_1
static spv_target_env defaultSPIRVTargetEnv = SPV_ENV_UNIVERSAL_1_3;
...
getInterfaceVariables(spirv::FuncOp funcOp,
  ...
  funcOp.walk([&](spirv::AddressOfOp addressOfOp) {
    ...
    switch (mlir::cast<spirv::PointerType>(var.getType()).getStorageClass()) {
...
getExternLibs(spirv::ModuleOp module) {
  ...
  for (auto &func : funcs) {
    if (func.getOperation()->hasAttr("libname")) {
      auto name =
          mlir::dyn_cast<StringAttr>(func.getOperation()->getAttr("libname"));
      auto path =
          mlir::dyn_cast<StringAttr>(func.getOperation()->getAttr("libpath"));
      if (name) {
  ...
  if (module.getOperation()->hasAttr("triton_gpu.externs")) {
    auto dict = mlir::dyn_cast<DictionaryAttr>(
    ...
    for (auto &attr : dict) {
      ...
      if (libName.compare("libdevice") != 0) {
        externLibs[libName] =
            mlir::dyn_cast<StringAttr>(attr.getValue()).strref().trim().str();
      }
    }
  }
  ...
  if (!funcs.empty()) {
    ...
    std::string env_path = triton::tools::getStrEnv("TRITON_LIBDEVICE_PATH");
    if (!env_path.empty()) {
...
static LogicalResult translateTritonSPIRVToSPIRVIR(ModuleOp module,
  ...
  module.walk([&](ModuleOp op) {
    ...
    newModuleOp->setAttr("vce_triple", spirv::VerCapExtAttr::get(
                                           spirv::Version::V_1_3, caps_opencl,
...
translateTritonGPUToSPIRVIR(mlir::ModuleOp module,
  ...
  // reason about this is : https://github.com/llvm/llvm-project/pull/95057
