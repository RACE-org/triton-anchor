...
struct GetNumProgramsOpSPIRVConversion
  ...
  matchAndRewrite(triton::GetNumProgramsOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Location loc = op->getLoc();
    assert((int)(op.getAxis()) < 3);

    Value blockId =
        rewriter.create<::mlir::gpu::GridDimOp>(loc, dims[(int)(op.getAxis())]);
...
struct AssertOpSPIRVConversion
  ...
  matchAndRewrite(triton::AssertOp op, OpAdaptor adaptor,
    ...
    for (auto &elem : elems) {
      if (elemTy.isSignedInteger() || elemTy.isSignlessInteger()) {
        ...
        // condition = and_(
        //     condition, icmp_slt(tid_val(), i32_val(sizePerCTA /
        //     numPerThread)));
      } else {
  ...
  void spirvAssert(Operation *op, Value condition, StringRef message,
    ...
    (llvm::Twine(file) + ": " + llvm::Twine(line) + ":Assertion " +
     llvm::Twine(message) + llvm::Twine("\n\0"))
        .toStringRef(msg);
    Value messageString =
        spirv::addStringToModule(loc, rewriter, "assert", msg);
    rewriter.create<mlir::spirv::CLPrintfOp>(
        UnknownLoc::get(rewriter.getContext()), i32_ty, messageString,
        ValueRange());

    // Split a block after the call.
    Block *thenBlock = rewriter.splitBlock(ifBlock, op->getIterator());
    rewriter.setInsertionPointToEnd(ifBlock);
    rewriter.create<mlir::spirv::ReturnOp>(loc);
...
void populateTritonGPUToSPIRVPatterns(
  ...
  // patterns.add<AllocTensorOpSPIRVConversion>(typeConverter, context,
  // allocation,
  //                                            benefit);
...
//     auto order =
//     resultTy.getEncoding().cast<SharedEncodingAttr>().getOrder();
...
//     auto smemObj = SharedMemoryObject(smemBase, resultTy.getShape(),
//     newOrder,
//                                       loc, rewriter);
...
//     assert(srcLayout && "Unexpected resultLayout in
//     ExtractSliceOpConversion"); assert(op.hasUnitStride() &&
...
//     auto spirvElemTy =
//     getTypeConverter()->convertType(srcTy.getElementType()); auto elemPtrTy =
//     ptr_ty(spirvElemTy, spirv::StorageClass::Workgroup); smemObj =
//     SharedMemoryObject(gep(elemPtrTy, smemObj.base, offset),
...
//     : public
//     ConvertTritonGPUOpToSPIRVPattern<triton::gpu::AsyncCommitGroupOp> {
//   using ConvertTritonGPUOpToSPIRVPattern<
