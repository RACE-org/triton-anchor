...
public:
  ...
  Value getSharedMemoryBase(Location loc, ConversionPatternRewriter &rewriter,
    ...
    if (bufferId == Allocation::InvalidBufferId) {
      if constexpr (std::is_pointer_v<T>) {
        if (value->getNumResults() == 1) {
          Value opvalue = value->getResult(0);
          bufferId = funcAllocation->getBufferId(opvalue);
        }
      }
    }
  ...
  // fantgpu: this swizzle maybe isn't suitable for fantgpu due to hardward
  // difference need further check
  ...
  getSwizzledSharedPtrs(Location loc, unsigned inVec, RankedTensorType srcTy,
    ...
    Value dstPtrBase = gep(smemObj.getTypes()[0], smemObj.base, dstOffset);
    dstPtrBase = bitcast(dstPtrBase, dstPtrTy);
    ...
    for (unsigned elemIdx = 0; elemIdx < numElems; elemIdx += minVec) {
      ...
      Value idxRow;                    // discontiguous dimension
      if (outOrder.size() >= 2) {
      ...
      if (auto add = dyn_cast_or_null<spirv::IAddOp>(idxCol.getDefiningOp()))
        if (auto _cst = dyn_cast_or_null<spirv::ConstantOp>(
          ...
          unsigned cst = mlir::cast<mlir::IntegerAttr>(_cst.getValue())
                             .getValue()
                             .getSExtValue();
      ...
      if (outOrder.size() == 3) {
        assert(outOrder[2] == 0 && "Unexpected rank of getSwizzledSharedPtrs");
      ...
      Value immediateOff;
      if (outOrder.size() >= 2) {
        immediateOff = add(mul(i32_val(immedateOffRow), strideRow),
  ...
  loadSharedToDistributed(Value dst, ArrayRef<SmallVector<Value>> dstIndices,
    ...
    if (auto mmaLayout =
            dyn_cast<NvidiaMmaEncodingAttr>(dstDistributedLayout)) {
  ...
  void storeDistributedToShared(Value src, Value llSrc,
    ...
    if (auto mmaLayout =
            dyn_cast<NvidiaMmaEncodingAttr>(srcDistributedLayout)) {
      assert((!mmaLayout.isVolta()) &&
    ...
    SmallVector<Value, 3> offsetVals(dstStrides.size(), i32_val(0));
  ...
  emitIndices(Location loc, ConversionPatternRewriter &b, Attribute layout,
    ...
    if (cache && cache->count(key) > 0) {
      ...
      if (auto blocked = mlir::dyn_cast<BlockedEncodingAttr>(layout)) {
        result = emitIndicesForDistributedLayout(loc, b, blocked, type,
                                                 withCTAOffset);
      } else if (auto slice = mlir::dyn_cast<SliceEncodingAttr>(layout)) {
        result =
