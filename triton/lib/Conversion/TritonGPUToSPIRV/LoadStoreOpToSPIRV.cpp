...
// Contains some helper functions for Load/Store and Atomic conversions.
struct LoadStoreSPIRVConversionBase {
  ...
  unsigned getContiguity(Value ptr) const {
    auto tensorTy = mlir::dyn_cast<RankedTensorType>(ptr.getType());
    if (!tensorTy)
      return 1;
  ...
  unsigned getVectorSize(Value ptr) const {
    ...
    // The maximum vector size is 128 bits on both Nvidia and Fant GPUs.
...
struct LoadOpSPIRVConversion
  ...
  matchAndRewrite(triton::LoadOp op, OpAdaptor adaptor,
    ...
    int64_t splatVal = 0;
    if (other && mlir::isa<IntegerType>(valueElemTy) &&
        matchPattern(other, m_Constant(&constAttr)) && constAttr.isSplat() &&
        mlir::isa<IntegerType>(constAttr.getElementType())) {
    ...
    for (size_t vecStart = 0; vecStart < numElems; vecStart += vec) {
      ...
      Value ret;
      if (op.getIsVolatile()) {
        ret = rewriter.create<spirv::LoadOp>(
            loc, ptrElem,
      ...
      for (unsigned int ii = 0; ii < nWords; ++ii) {
        Value curr;
        if (mlir::isa<mlir::VectorType>(retTy)) {
...
struct StoreOpSPIRVConversion
  ...
  matchAndRewrite(triton::StoreOp op, OpAdaptor adaptor,
    ...
    auto tensorTy = mlir::dyn_cast<RankedTensorType>(valueTy);
...
static mlir::spirv::MemorySemantics getMemoryOrdering(MemSemantic memOrdering) {
  switch (memOrdering) {
  case MemSemantic::RELAXED:
    return mlir::spirv::MemorySemantics::None |
           mlir::spirv::MemorySemantics::MakeAvailable |
           mlir::spirv::MemorySemantics::MakeVisible;
  case MemSemantic::ACQUIRE:
    return mlir::spirv::MemorySemantics::Acquire |
           mlir::spirv::MemorySemantics::MakeAvailable |
           mlir::spirv::MemorySemantics::MakeVisible;
  case MemSemantic::RELEASE:
    return mlir::spirv::MemorySemantics::Release |
           mlir::spirv::MemorySemantics::MakeAvailable |
           mlir::spirv::MemorySemantics::MakeVisible;
  case MemSemantic::ACQUIRE_RELEASE:
    return mlir::spirv::MemorySemantics::AcquireRelease |
           mlir::spirv::MemorySemantics::MakeAvailable |
           mlir::spirv::MemorySemantics::MakeVisible;
  default:
    return mlir::spirv::MemorySemantics::AcquireRelease |
...
static mlir::spirv::Scope getMemoryScope(MemSyncScope val) {
  switch (val) {
  case MemSyncScope::GPU:
    return mlir::spirv::Scope::Device;
  case MemSyncScope::CTA:
    return mlir::spirv::Scope::Workgroup;
  case MemSyncScope::SYSTEM:
    return mlir::spirv::Scope::CrossDevice;
  }
  return mlir::spirv::Scope::Device;
}
...
struct AtomicCASOpSPIRVConversion
  ...
  matchAndRewrite(triton::AtomicCASOp op, OpAdaptor adaptor,
    ...
    auto ptrElements = typeConverter->unpackLLElements(loc, spirvPtr, rewriter,
                                                       op.getPtr().getType());
    auto cmpElements = typeConverter->unpackLLElements(loc, spirvCmp, rewriter,
                                                       op.getCmp().getType());
    auto valElements = typeConverter->unpackLLElements(loc, spirvVal, rewriter,
                                                       op.getVal().getType());
    ...
    if (valueElemTy.isF32() ||
        valueElemTy.isF16()) // float cannot supported by fant gpu
      return failure();
    ...
    for (size_t i = 0; i < elemsPerThread; i += 1) {
      ...
      if (TensorTy) { // for tensor
        auto cmpxchg = rewriter.create<spirv::AtomicCompareExchangeOp>(
            loc, valueElemTy, casPtr, getMemoryScope(op.getScope()),
            getMemoryOrdering(op.getSem()), getMemoryOrdering(op.getSem()),
            casVal, casCmp);

        resultVals[i] = cmpxchg;
      } else {
        ...
        // Test the prediction
        rewriter.setInsertionPoint(preheader, preheader->end());
        rewriter.create<mlir::cf::CondBranchOp>(loc, pred, condblock,
                                                tailblock);

        // Do the Atomic
        rewriter.setInsertionPoint(condblock, condblock->end());

        Value old = rewriter.create<spirv::AtomicCompareExchangeOp>(
            loc, valueElemTy, casPtr, getMemoryScope(op.getScope()),
            getMemoryOrdering(op.getSem()), getMemoryOrdering(op.getSem()),
            casVal, casCmp);
    ...
    if (TensorTy) {
      ...
      Value resultStruct =
          typeConverter->packLLElements(loc, resultVals, rewriter, structTy);
      rewriter.replaceOp(op, {resultStruct});
    }
    return success();
  }
...
struct AtomicRMWOpSPIRVConversion
  ...
  matchAndRewrite(triton::AtomicRMWOp op, OpAdaptor adaptor,
    ...
    auto typeConverter = getTypeConverter();
    auto valElements =
        typeConverter->unpackLLElements(loc, spirvVal, rewriter, val.getType());
    auto ptrElements =
        typeConverter->unpackLLElements(loc, spirvPtr, rewriter, ptr.getType());
    SmallVector<Value> maskElements;
    if (spirvMask)
      maskElements = typeConverter->unpackLLElements(loc, spirvMask, rewriter,
                                                     op.getMask().getType());
...
#define DISPATCH(rwmop__, sprivop__)                                           \
  case (rwmop__):                                                              \
    ret = rewriter.create<sprivop__>(loc, retType, ptrElem,                    \
                                     getMemoryScope(op.getScope()),            \
                                     getMemoryOrdering(op.getSem()), rmwVal);  \
    break;
...
//     assert(srcLayout && "Unexpected srcLayout in
//     InsertSliceOpSPIRVConversion");
...
//     assert(dstLayout && "Unexpected dstLayout in
//     InsertSliceOpSPIRVConversion"); assert(op.hasUnitStride() &&
...
//       // assert(false && "insert_slice_async: Other value not supported
//       yet"); otherElems = getTypeConverter()->unpackLLElements(
//           loc, spirvOther, rewriter, other.getType());
//       assert(srcElems.size() == otherElems.size());
//     }

//     // We don't use getVec() here because we are copying from memory to
//     memory.
...
//           spirv::PointerType::get(spirvElemTy,
//           spirv::StorageClass::Workgroup);
//       basePtr = bitcast(basePtr, spirvBasePtrType);
//       for (size_t wordIdx = 0; wordIdx < numWords; ++wordIdx) {
//         Value spirvDestPtr = gep(spirvBasePtrType, basePtr,
//         i32_val(wordIdx));
...
//           rewriter.setInsertionPoint(condblock, condblock->end());
//           Value val = rewriter.create<spirv::LoadOp>(op.getLoc(),
//           spirvSrcPtr); rewriter.create<mlir::cf::BranchOp>(loc, tailblock,
//           ValueRange{val});
...
//         // the cp.async is treated as a weak memory operation in the CUDA
//         memory
...
struct LocalAllocOpConversion
  ...
  LocalAllocOpConversion(TritonGPUToSPIRVTypeConverter &converter,
                         MLIRContext *context, ModuleAllocation &allocation,
                         ModuleAxisInfoAnalysis &axisAnalysisPass,
                         PatternBenefit benefit)
      : ConvertTritonGPUOpToSPIRVPattern<triton::gpu::LocalAllocOp>(
            converter, context, allocation, benefit) {}
  ...
  matchAndRewrite(triton::gpu::LocalAllocOp op, OpAdaptor adaptor,
    ...
    smemBase = bitcast(smemBase, ptr_ty(srcTy.getElementType(),
                                        spirv::StorageClass::Workgroup));
    ...
    if (op.getSrc()) {
      lowerDistributedToShared(op, adaptor, rewriter);
    }
  ...
  void lowerDistributedToShared(triton::gpu::LocalAllocOp op,
                                triton::gpu::LocalAllocOpAdaptor adaptor,
    ...
    auto outOrd =
        mlir::cast<SharedEncodingAttr>(dstTy.getEncoding()).getOrder();
    ...
    storeDistributedToShared(op.getSrc(), adaptor.getSrc(), dstStrides,
                             srcIndices, op.getResult(), smemBase, elemTy, loc,
                             rewriter);
    return;
  }
};
...
void populateLoadStoreOpToSPIRVPatterns(
  ...
  patterns.add<AtomicRMWOpSPIRVConversion>(typeConverter, context, allocation,
                                           axisInfoAnalysis, benefit);
  patterns.add<LocalAllocOpConversion>(typeConverter, context, allocation,
                                       axisInfoAnalysis, benefit);
  // patterns.add<InsertSliceOpSPIRVConversion>(typeConverter, context,
  // allocation,
