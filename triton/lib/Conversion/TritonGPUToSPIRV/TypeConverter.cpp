...
Value TritonGPUToSPIRVTypeConverter::packLLElements(
    ...
    Type type) {
  auto structType = mlir::dyn_cast<spirv::StructType>(this->convertType(type));
  if (!structType) {
...
SmallVector<Value> TritonGPUToSPIRVTypeConverter::unpackLLElements(
    ...
    Type type) { // this type is not used
  assert(bool(spirvStruct) && "can not unpack null values");
  if (spirvStruct.getType().isIntOrIndexOrFloat() ||
      mlir::isa<triton::PointerType>(spirvStruct.getType()) ||
      mlir::isa<spirv::PointerType>(spirvStruct.getType()))
    return {spirvStruct};
  auto types =
...
Type TritonGPUToSPIRVTypeConverter::convertMemDescType(MemDescType type) {
  ...
  SmallVector<Type, 4> types;
  // base ptr
  auto ptrType = spirv::PointerType::get(type.getElementType(),
