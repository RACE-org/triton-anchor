...
#include "lib/Conversion/TritonGPUToSPIRV/Utility.h"
...
getSharedMemoryObjectFromStruct(Location loc, Value spirvStruct,
  ...
  auto types =
      mlir::cast<spirv::StructType>(spirvStruct.getType()).getElementTypes();
...
Value loadShared(ConversionPatternRewriter &rewriter, Location loc, Value ptr,
                 Value pred) {
  auto ptrTy = mlir::cast<spirv::PointerType>(ptr.getType());
...
Value addStringToModule(Location loc, ConversionPatternRewriter &rewriter,
  ...
  auto varOps = moduleOp.getBody()->getOps<spirv::GlobalVariableOp>();
  std::string numsGlobalVar =
  ...
  spirv::SpecConstantCompositeOp specCstComposite;
  {
    ...
    for (auto c : contentWithNull) {
      ...
      (llvm::Twine(key) + llvm::Twine(numsGlobalVar) + "_speccst" +
       llvm::Twine(specConstIdx++))
          .toVector(specCstName);
    ...
    (llvm::Twine(key) + llvm::Twine(numsGlobalVar) + "_speccstcomp")
        .toVector(specCstCompositeName);
