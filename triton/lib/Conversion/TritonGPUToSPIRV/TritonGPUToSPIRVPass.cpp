  ...
  matchAndRewrite(triton::FuncOp funcOp, OpAdaptor adaptor,
    ...
    if (allocation.isRoot(funcOp)) {
      ...
      newFuncOp->setAttr(spirv::getEntryPointABIAttrName(),
                         spirv::EntryPointABIAttr::get(getContext(), nullptr,
                                                       std::nullopt, 32));
    } else {
...
struct ConvertDivRZOp: public OpRewritePattern<triton::DivRZOp>
{
  using OpRewritePattern::OpRewritePattern;
  LogicalResult matchAndRewrite(triton::DivRZOp op, PatternRewriter &rewriter) const override {
    auto loc = op.getLoc();
    Value lhs = op.getX();
    Value rhs = op.getY();
    Type res_type = op.getType();
    if (!cast<RankedTensorType>(res_type).getElementType().isF32()) {
      return failure();
    }
    // TODO:先用div_rn近似表示rz mode
    auto newOp = rewriter.create<triton::PreciseDivFOp>(loc, res_type, lhs, rhs);
...
public:
  ...
  void runOnOperation() override {
    ...
    spirv::Capability caps_opencl[] = {
        spirv::Capability::Addresses, spirv::Capability::Float16Buffer,
        spirv::Capability::Int64, spirv::Capability::Int16,
        spirv::Capability::Int8, spirv::Capability::Kernel,
        spirv::Capability::Linkage, spirv::Capability::GroupNonUniformShuffle,
        // spirv::Capability::Vector16,
        // spirv::Capability::GenericPointer,
        // spirv::Capability::Groups,
        spirv::Capability::Float16, spirv::Capability::Float64,
    ...
    auto triple =
        spirv::VerCapExtAttr::get(spirv::Version::V_1_3, caps_opencl,
    ...
    // Preprocess
    // removed by fantgpu temporarily
    ...
    // builtin call ops
    RewritePatternSet builtin_ops_patterns(context);
    // TypeConverter typeConverter;
    builtin_ops_patterns.add<ConvertDivRZOp>(context);
    mlir::ConversionTarget builtin_ops_target(*context);
    builtin_ops_target.markUnknownOpDynamicallyLegal([](Operation *) { return true; });
    // fdiv_rz
    builtin_ops_target.addIllegalOp<triton::DivRZOp>();
    // mod->dump();
    if (failed(
            applyPartialConversion(mod, builtin_ops_target, std::move(builtin_ops_patterns))))
      return signalPassFailure();
    // llvm::errs() << "********************************[after ConvertDivRZOp ir is]********************************\n";
    // mod->dump();
    ...
    // llvm::errs() << "complete populateTritonGPUToSPIRVPatterns...\n";
    // mod->dump();
    // ConvertLayoutOp
    ...
    populateReduceOpToSPIRVPatterns(spirvTypeConverter, context, patterns,
                                    ...
                                    indexCacheInfo, /*benefit=*/10);
    // HistogramOp
    populateHistogramOpToSPIRVPatterns(spirvTypeConverter, context, patterns,
                                       numWarps, axisInfoAnalysis, allocation,
                                       indexCacheInfo, /*benefit=*/10);
    ...
    // PrintfOp
    populatePrintOpToSPIRVPattern(spirvTypeConverter, context, patterns,
                                  /*benefit=*/10);
...
private:
  ...
  // void decomposeMmaToDotOperand(ModuleOp mod, int numWarps, int
  // threadsPerWarp,
  //                               int numCTAs) const {
  ...
  //     auto srcType =
  //     mlir::cast<RankedTensorType>(cvtOp.getOperand().getType()); auto
  //     dstType = mlir::cast<RankedTensorType>(cvtOp.getType()); auto srcMma =
  //         mlir::dyn_cast<triton::gpu::NvidiaMmaEncodingAttr>(srcType.getEncoding());
  //     auto dstDotOp =
  //         mlir::dyn_cast<triton::gpu::DotOperandEncodingAttr>(dstType.getEncoding());
  ...
  //     auto srcType =
  //     mlir::cast<RankedTensorType>(cvtOp.getOperand().getType()); auto
  //     dstType = mlir::cast<RankedTensorType>(cvtOp.getType()); auto
  //     srcBlocked =
  //         mlir::dyn_cast<triton::gpu::BlockedEncodingAttr>(srcType.getEncoding());
  //     auto dstDotOp =
  //         mlir::dyn_cast<triton::gpu::DotOperandEncodingAttr>(dstType.getEncoding());
  ...
  //   ModuleAxisInfoAnalysis axisInfoAnalysis(mod);
  //   // TODO(Keren): This is a hacky knob that may cause performance
  //   regression
  //   // when decomposition has been performed. We should remove this knob once
  //   we
  //   // have thorough analysis on async wait. Currently, we decompose
  //   // `insert_slice_async` into `load` and `insert_slice` without knowing
  //   which
  //   // `async_wait` is responsible for the `insert_slice_async`. To guarantee
  //   // correctness, we blindly set the `async_wait` to wait for all async
  //   ops.
  //   //
  ...
  //     auto srcTy = mlir::cast<RankedTensorType>(src.getType());
  //     auto dstTy = mlir::cast<RankedTensorType>(dst.getType());
  //     auto srcBlocked =
  //         mlir::dyn_cast<triton::gpu::BlockedEncodingAttr>(srcTy.getEncoding());
  //     auto resSharedLayout =
  //         mlir::dyn_cast<triton::gpu::SharedEncodingAttr>(dstTy.getEncoding());
  //     auto resElemTy = dstTy.getElementType();
  //     unsigned inVec = axisInfoAnalysis.getPtrContiguity(src);
  //     if (mask)
  //       inVec =
  //           std::min<unsigned>(axisInfoAnalysis.getMaskAlignment(mask),
  //           inVec);
