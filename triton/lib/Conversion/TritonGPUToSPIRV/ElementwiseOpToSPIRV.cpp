...
inline SmallVector<Value> unpackI32(const SmallVector<Value> &inValues,
  ...
  if (!(encoding && mlir::isa<NvidiaMmaEncodingAttr>(encoding.getParent())))
    return inValues;
...
//     // llvm::outs() << 0 << "\n";
//     auto srcTensorType =
//     op.getFrom().getType().cast<mlir::RankedTensorType>(); auto dstTensorType
//     =
...
//     // Unpack value
//     auto inVals = getTypeConverter()->unpackLLElements(loc,
//     adaptor.getFrom(),
//                                                        rewriter,
//                                                        srcTensorType);
//     inVals =
...
struct ClampFOpSPIRVConversion
    : public ElementwiseOpSPIRVConversionBase<ClampFOp,
                                              ClampFOpSPIRVConversion> {
  using Base =
      ElementwiseOpSPIRVConversionBase<ClampFOp, ClampFOpSPIRVConversion>;
  using Base::Base;
  using Adaptor = typename Base::OpAdaptor;

  // An interface to support variant DestOp builder.
  Value createDestOp(ClampFOp op, OpAdaptor adaptor,
                     ConversionPatternRewriter &rewriter, Type elemTy,
                     ValueRange operands, Location loc) const {
    // Clip pattern not found, use min/max.
    if (op.getPropagateNan() == PropagateNan::ALL) {
      // we need to handle NaN propagation
      // manually. We need to check only the first operand for clamp.
      auto lhs = operands[0];
      auto isNan = rewriter.create<arith::CmpFOp>(
          loc, arith::CmpFPredicate::UNE, lhs, lhs);
      auto v = rewriter.create<arith::MaxNumFOp>(loc, elemTy, operands[0],
                                                 operands[1]);
      auto nonNanRes = rewriter.create<arith::MinNumFOp>(loc, v, operands[2]);
      // Select the result based on the isNan flag.
      return rewriter.create<arith::SelectOp>(loc, isNan, lhs, nonNanRes);
    }
    ...
    auto v = rewriter.create<arith::MaxNumFOp>(loc, elemTy, operands[0],
                                               operands[1]);
    return rewriter.create<arith::MinNumFOp>(loc, v, operands[2]);
  }
};

struct MulhiUIOpSPIRVConversion
    : public ElementwiseOpSPIRVConversionBase<MulhiUIOp,
                                              MulhiUIOpSPIRVConversion> {
  using Base =
      ElementwiseOpSPIRVConversionBase<MulhiUIOp, MulhiUIOpSPIRVConversion>;
  using Base::Base;
  using Adaptor = typename Base::OpAdaptor;

  // An interface to support variant DestOp builder.
  Value createDestOp(MulhiUIOp op, OpAdaptor adaptor,
                     ConversionPatternRewriter &rewriter, Type elemTy,
                     ValueRange operands, Location loc) const {
    // Ensure we have exactly two operands
    assert(operands.size() == 2 && "umulhi requires exactly two operands");
    ...
    if (elemTy.isUnsignedInteger()) {
      return rewriter.create<spirv::CLUmulHiOp>(loc, lhs, rhs);
    } else {
      return rewriter.create<spirv::CLSmulHiOp>(loc, lhs, rhs);
    }

    // following method report error: unsupported code 152
    // // Create the SPIR-V instruction for UMulExtended
    // auto structType = spirv::StructType::get({elemTy, elemTy});
    // Value umulExtended = rewriter.create<spirv::SMulExtendedOp>(loc,
    // structType, lhs, rhs);

    // // Extract the high part of the product
    // Value high = rewriter.create<spirv::CompositeExtractOp>(loc, elemTy,
    // umulExtended, rewriter.getI32ArrayAttr(1)); return high;
  }
};
...
//     spirv::FuncOp funcOp = appendOrGetFuncOp(rewriter, op, funcName,
//     funcType);
...
// why not use spirv::IMulOp directly???
// because if input of IMulOp is i1, arith::MulIOp is converted into
// spirv::IMulOp(i1, i1) but spirv::IMulOp cannot support i1. It can support
// i8/i16/i32/i64, but not i1.
struct IMulOpSPIRVConversion
    : ElementwiseOpSPIRVConversionBase<mlir::arith::MulIOp,
                                       IMulOpSPIRVConversion> {
  using Base = ElementwiseOpSPIRVConversionBase<mlir::arith::MulIOp,
                                                IMulOpSPIRVConversion>;
  using Base::Base;
  using Adaptor = typename Base::OpAdaptor;

  Value createDestOp(mlir::arith::MulIOp op, OpAdaptor adaptor,
                     ConversionPatternRewriter &rewriter, Type elemTy,
                     ValueRange operands, Location loc) const {
    auto lhsElemTy = getElementType(op.getLhs());
    auto rhsElemTy = getElementType(op.getRhs());
    if ((lhsElemTy.getIntOrFloatBitWidth() == 1 &&
         rhsElemTy.getIntOrFloatBitWidth() == 1)) {
      return rewriter.create<spirv::LogicalAndOp>(loc, elemTy, operands[0],
                                                  operands[1]);
    } else {
      return rewriter.create<spirv::IMulOp>(loc, elemTy, operands[0],
                                            operands[1]);
    }
  }
};
...
struct ExpOpSPIRVConversionApprox
  ...
  Value createDestOp(mlir::math::ExpOp op, OpAdaptor adaptor,
                     ConversionPatternRewriter &rewriter, Type elemTy,
                     ValueRange operands, Location loc) const {
    // For non-FP32 input, call math::ExpOp for higher-precision calculation
...
struct TruncOpConversion
    : ElementwiseOpSPIRVConversionBase<mlir::math::TruncOp, TruncOpConversion> {
  using Base =
      ElementwiseOpSPIRVConversionBase<mlir::math::TruncOp, TruncOpConversion>;
  using Base::Base;
  using Adaptor = typename Base::OpAdaptor;

  Value createDestOp(mlir::math::TruncOp op, OpAdaptor adaptor,
                     ConversionPatternRewriter &rewriter, Type elemTy,
                     ValueRange operands, Location loc) const {
    auto int_Ty = IntegerType::get(op.getContext(),
                                   mlir::cast<FloatType>(elemTy).getWidth());
    // 向 0 截断
    auto toInt = rewriter.create<spirv::ConvertFToSOp>(loc, int_Ty, operands[0]);
    return rewriter.create<spirv::ConvertSToFOp>(loc, elemTy, toInt.getResult());
  }
};
...
#define POPULATE_TERNARY_OP(SRC_OP, DST_OP)                                    \
  ...
  POPULATE_TERNARY_OP(mlir::math::FmaOp, mlir::math::FmaOp)
...
#define POPULATE_BINARY_OP(SRC_OP, DST_OP)                                     \
  ...
  POPULATE_BINARY_OP(arith::MaxNumFOp, arith::MaxNumFOp)   // fmax
  POPULATE_BINARY_OP(arith::MinNumFOp, arith::MinNumFOp)   // fmin
  ...
  POPULATE_BINARY_OP(triton::PreciseDivFOp, arith::DivFOp) // fdiv_rn
  POPULATE_BINARY_OP(triton::PreciseSqrtOp, math::SqrtOp)  // rsqrt_rn
  patterns.add<MulhiUIOpSPIRVConversion>(typeConverter, context, benefit);
...
#define POPULATE_UNARY_OP(SRC_OP, DST_OP)                                      \
  ...
  POPULATE_UNARY_OP(math::IsFiniteOp, math::IsFiniteOp)
  POPULATE_UNARY_OP(math::RoundEvenOp, math::RoundEvenOp)
  POPULATE_UNARY_OP(math::IsInfOp, math::IsInfOp)
  POPULATE_UNARY_OP(math::IsNaNOp, math::IsNaNOp)
...
#undef POPULATE_UNARY_OP
  ...
  patterns.add<ClampFOpSPIRVConversion>(typeConverter, context, benefit);
  ...
  patterns.add<FMulOpSPIRVConversion>(
      ...
      mlir::spirv::checkOpSupported(computeCapability,
                                    "INTELConvertFToBF16Op"));
  patterns.add<IMulOpSPIRVConversion>(
      typeConverter, context, benefit,
      mlir::spirv::checkOpSupported(computeCapability,
                                    "INTELConvertFToBF16Op"));
  ...
  patterns.add<TruncOpConversion>(typeConverter, context, benefit);
}
