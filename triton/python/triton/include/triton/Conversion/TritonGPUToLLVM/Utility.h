...
#ifndef NO_TTGIR
using ::mlir::triton::gpu::FANTWmmaEncodingAttr;
#endif // NO_TTGIR
...
#ifndef NO_TTGIR
inline void emitFANTWmmaOffsetForCTA(const FANTWmmaEncodingAttr &wmmaLayout,
                                 SmallVector<SmallVector<unsigned>> &offsets,
                                 unsigned ctaBatchOffset, unsigned ctaOffsetX,
                                 unsigned ctaOffsetY) {
  const unsigned elemsPerThreadPerGroup = 2;
  auto warpSize = getWarpSize(wmmaLayout);
  assert(warpSize == 128);
  auto shapePerCta = getShapePerCTATile(wmmaLayout);
  auto rank = shapePerCta.size();
  ...
  if (rank == 3)
    elemOffset[0] = ctaBatchOffset;
  for (unsigned elem = 0; elem < elemsPerThreadPerGroup; elem++) {
    elemOffset[rank - 2] = ctaOffsetX * shapePerCta[rank - 2];
    elemOffset[rank - 1] = ctaOffsetY * shapePerCta[rank - 1] + elem;
    offsets.push_back(elemOffset);
  }
}
...
#ifndef NO_TTGIR
inline SmallVector<Value>
emitBaseIndexForFANTWmmaLayout(Location loc, RewriterBase &rewriter,
                           const FANTWmmaEncodingAttr &wmmaLayout,
                           RankedTensorType type) {
  auto shape = type.getShape();
  auto _warpsPerCTA = wmmaLayout.getWarpsPerCTA();
  auto rank = _warpsPerCTA.size();
  ...
  for (unsigned i = 0; i < rank; ++i)
    warpsPerCTA.push_back(i32_val(_warpsPerCTA[i]));
  auto mnkDim = FANTWmmaEncodingAttr::getMNKDimPerWMMAInstr();
  ...
  multiDimBase[rank - 2] =
      add(udiv(threadIdPerWarp, i32_val(8)), offWarp0);
  multiDimBase[rank - 1] = 
      add(mul(urem(threadIdPerWarp, i32_val(8)), i32_val(2)), offWarp1);
...
#ifndef NO_TTGIR
inline SmallVector<SmallVector<unsigned>>
emitOffsetForFANTWmmaLayout(const FANTWmmaEncodingAttr &wmmaLayout,
                        RankedTensorType type) {
  auto tensorShape = type.getShape();
  SmallVector<SmallVector<unsigned>> offsets;
  auto shapePerCTA = getShapePerCTA(wmmaLayout, tensorShape);
  ...
  for (unsigned b = 0; b < repBatch; ++b) {
    for (unsigned i = 0; i < repM; ++i) {
      for (unsigned j = 0; j < repN; ++j) {
        emitFANTWmmaOffsetForCTA(wmmaLayout, offsets, b * warpsPerBatch, i, j);
      }
    }
  }
  return offsets;
...
#ifndef NO_TTGIR
  } else if (auto wmmaLayout = mlir::dyn_cast<FANTWmmaEncodingAttr>(layout)) {
    result = emitBaseIndexForFANTWmmaLayout(loc, rewriter, wmmaLayout, type);
#endif // NO_TTGIR
...
#ifndef NO_TTGIR
  if (auto wmmaLayout = mlir::dyn_cast<FANTWmmaEncodingAttr>(layout)) {
    return emitOffsetForFANTWmmaLayout(wmmaLayout, type);
  }
