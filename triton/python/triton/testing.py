...
if not _NO_TTGIR:
    ...
    def do_bench_ocl(fn, warmup=25, rep=100, grad_to_none=None, quantiles=None, fast_flush=True, return_mode="mean"):
        """
    ...
    """
        ...
        if fast_flush:
            cache = torch.empty(int(256e6 // 4), dtype=torch.int, device='ocl:0')
        else:
            cache = torch.empty(int(256e6), dtype=torch.int8, device='ocl:0')
        ...
        estimate_ms = (end_time - start_time) * 1000 / 5
        ...
        start_times = [0] * n_repeat
        end_times = [0] * n_repeat
        for i in range(n_repeat):
            if grad_to_none is not None:
                for x in grad_to_none:
                    x.grad = None
        ...
        times = torch.tensor([(e - s) * 1000 for s, e in zip(start_times, end_times)], dtype=torch.float)
    ...
    def do_bench_ocl_event(fn, warmup=25, rep=100, grad_to_none=None, quantiles=None, fast_flush=True, return_mode="mean"):
        """
    Benchmark the runtime of the provided function. By default, return the median runtime of :code:`fn` along with
    the 20-th and 80-th performance percentile.
    ...
    """
        ...
        # doesn't contain any input data before the run
        if fast_flush:
            cache = torch.empty(int(16e6 // 4), dtype=torch.int, device='ocl:0')
        else:
            cache = torch.empty(int(16e6), dtype=torch.int8, device='ocl:0')

        torch.ocl.enable_time("ocl:0")
        for _ in range(5):
            cache.zero_()
            torch.ocl.empty_cache()
            fn()
        ...
        estimate_ms = torch.ocl.elapsed_time("ocl:0")
        n_warmup = max(1, int(warmup / estimate_ms))
        n_repeat = max(1, int(rep / estimate_ms))
        ...
        for i in range(n_repeat):
            ...
            torch.ocl.enable_time("ocl:0")
            fn()
            end_times[i] = torch.ocl.elapsed_time("ocl:0")

        torch.ocl.synchronize()
        times = torch.tensor([t for t in end_times], dtype=torch.float)
        if quantiles is not None:
            ret = torch.quantile(times, torch.tensor(quantiles, dtype=torch.float)).tolist()
            if len(ret) == 1:
                ret = ret[0]
...
if not _NO_TTGIR:
    ...
    """
        ...
        fn()
        torch.fant.synchronize()

        # We maintain a buffer of 256 MB that we clear
        # before each kernel call to make sure that the L2
        # doesn't contain any input data before the run
        if fast_flush:
            cache = torch.empty(int(256e6 // 4), dtype=torch.int, device='fant')
        else:
            cache = torch.empty(int(256e6), dtype=torch.int8, device='fant')

        # Estimate the runtime of the function
        start_event = torch.fant.Event(enable_timing=True)
        end_event = torch.fant.Event(enable_timing=True)
        start_event.record()
        for _ in range(5):
            cache.zero_()
            fn()
        ...
        n_warmup = max(1, int(warmup / estimate_ms))
        n_repeat = max(1, int(rep / estimate_ms))
        start_event = [torch.fant.Event(enable_timing=True) for i in range(n_repeat)]
        end_event = [torch.fant.Event(enable_timing=True) for i in range(n_repeat)]
        # Warm-up
        for _ in range(n_warmup):
            fn()
        # Benchmark
