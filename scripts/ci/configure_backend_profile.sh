...
case "${BACKEND_PROFILE}" in
  ...
  sophgo-cmodel)
    ...
    ci_default FLAGGEMS_TEST_FILES "tests/test_unary_pointwise_ops.py tests/test_binary_pointwise_ops.py tests/test_reduction_ops.py"
    ci_default FLAGGEMS_TEST_MARKERS "abs or add or sum"
